...
const testimonials = [
  {
    quote: "Booking an appointment took less than two minutes, and the reminders meant I never missed a follow-up.",
    name: "Emily Carter",
    role: "Patient",
  },
  {
    quote: "As a working parent, being able to check doctor availability in real time has been a genuine time-saver.",
    name: "Daniel Reyes",
    role: "Patient",
  },
  {
    quote: "The platform keeps my schedule organized and lets me review patient history before every visit.",
...
const FaqItem = ({ item, isOpen, onClick }) => (
  <div className="surface-card overflow-hidden">
    ...
    >
      <span className="font-semibold text-ink-900 text-sm sm:text-base">{item.q}</span>
      <span
...
const Home = () => {
  ...
  return (
    <div className="animate-fade-in">
      {/* ---------------- Hero ---------------- */}
      <section className="relative overflow-hidden bg-brand-900" style={{ background: "linear-gradient(135deg, var(--color-brand-800), var(--color-brand-600))" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 md:py-28 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center relative z-10">
          <div className="text-center lg:text-left animate-fade-up">
            ...
            <p className="text-base sm:text-lg text-brand-50/90 mb-8 max-w-xl mx-auto lg:mx-0">
              Connect with verified doctors across every specialty, book appointments
              instantly, and manage your care securely — all in one place.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
              <Link to="/doctors" className="btn-primary bg-white! text-brand-700! hover:bg-brand-50! shadow-lg">
                Find a Doctor
              </Link>
      ...
      <section className="py-10 sm:py-12 border-b border-ink-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 items-center opacity-80">
            {["St. Luke's General", "Riverside Medical", "Cedar Health", "Unity Clinics", "Northgate Care"].map((h) => (
              <p key={h} className="text-center text-sm sm:text-base font-display font-semibold text-ink-600">
                {h}
              </p>
            ))}
          </div>
      ...
      {/* ---------------- Stats ---------------- */}
      <section className="py-14 sm:py-16" style={{ background: "var(--color-brand-50)" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {stats.map((s) => (
            <div key={s.label}>
              <p className="text-3xl sm:text-4xl font-display font-extrabold text-brand-700">{s.value}</p>
              <p className="text-xs sm:text-sm text-ink-600 mt-1">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
            ...
            <p className="text-ink-600 text-sm sm:text-base">
              Everything you need for accessible, transparent, and secure healthcare.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {[
              { icon: "✅", title: "Verified Doctors", desc: "Every doctor is credential-checked and reviewed before joining the platform." },
              { icon: "⚡", title: "Instant Booking", desc: "Real-time availability means no phone calls and no waiting for confirmation." },
              { icon: "🔒", title: "Secure Payments", desc: "Stripe-powered checkout keeps your payment and medical data fully protected." },
              { icon: "📋", title: "Digital Records", desc: "Access prescriptions and medical history anytime from your dashboard." },
              { icon: "🔔", title: "Smart Reminders", desc: "Automatic notifications keep you on top of upcoming appointments." },
              { icon: "💬", title: "24/7 Support", desc: "Our support team is available around the clock for urgent questions." },
            ].map((f) => (
              <div key={f.title} className="surface-card p-6 sm:p-8 hover:shadow-lg transition-shadow">
                <div className="w-12 h-12 rounded-xl bg-brand-50 flex items-center justify-center text-2xl mb-4">
                  {f.icon}
                </div>
                <h3 className="text-lg font-semibold text-ink-900 mb-2">{f.title}</h3>
      ...
      {/* ---------------- Departments ---------------- */}
      <section className="py-16 sm:py-20 md:py-24" style={{ background: "var(--color-ink-50)" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold text-ink-900 mb-4">
              Medical Departments
            ...
            <p className="text-ink-600 text-sm sm:text-base">
              Find the right specialist for your needs across our full range of departments.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {departments.map((d) => (
              ...
              >
                ...
                <h3 className="font-semibold text-ink-900 text-sm sm:text-base">{d.name}</h3>
                <p className="text-xs sm:text-sm text-ink-400 mt-1">{d.desc}</p>
              </Link>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-12 sm:mb-16 gap-4">
            <div>
              ...
              <p className="text-ink-600 text-sm sm:text-base max-w-xl">
                Highly rated specialists ready to take care of you and your family.
              </p>
            </div>
            <Link to="/doctors" className="btn-secondary shrink-0 self-start sm:self-auto">
              View All Doctors
            </Link>
          </div>
      ...
      <section className="py-16 sm:py-20 md:py-24" style={{ background: "var(--color-ink-50)" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-6 relative">
            {[
              { step: "01", title: "Choose a Doctor", desc: "Browse by department, availability, or rating to find the right fit." },
              { step: "02", title: "Pick a Time Slot", desc: "Select an available slot that fits your schedule in real time." },
              { step: "03", title: "Confirm & Pay", desc: "Secure your appointment instantly with encrypted online payment." },
            ].map((s) => (
              <div key={s.step} className="surface-card p-6 sm:p-8 text-center relative">
      ...
      <section className="py-16 sm:py-20 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: "In-Person Consultations", desc: "Face-to-face visits with specialists at partner clinics." },
              { title: "Follow-Up Scheduling", desc: "Easily rebook follow-ups directly from your dashboard." },
              { title: "Digital Prescriptions", desc: "Receive and store prescriptions securely online." },
              { title: "Health Records Access", desc: "View your appointment and treatment history anytime." },
            ].map((s) => (
              <div key={s.title} className="p-6 rounded-2xl border border-ink-100 hover:border-brand-200 hover:bg-brand-50/40 transition-colors">
                <h3 className="font-semibold text-ink-900 mb-2 text-sm sm:text-base">{s.title}</h3>
                <p className="text-xs sm:text-sm text-ink-600 leading-relaxed">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ---------------- Testimonials ---------------- */}
      <section className="py-16 sm:py-20 md:py-24" style={{ background: "linear-gradient(135deg, var(--color-brand-800), var(--color-brand-700))" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
      ...
      <section className="py-16 sm:py-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl px-6 sm:px-12 py-12 sm:py-16 text-center" style={{ background: "linear-gradient(135deg, var(--color-brand-600), var(--color-accent-500))" }}>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold text-white mb-4">
              Ready to take control of your healthcare?
            </h2>
            <p className="text-brand-50/90 text-sm sm:text-base mb-8 max-w-xl mx-auto">
              Join thousands of patients booking appointments with trusted doctors every day.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link to="/register" className="btn-primary bg-white! text-brand-700! hover:bg-brand-50!">
                Get Started Free
              </Link>
              <Link to="/doctors" className="btn-secondary bg-transparent! text-white! border-white/30!">
                Browse Doctors
              </Link>
            </div>
          </div>
